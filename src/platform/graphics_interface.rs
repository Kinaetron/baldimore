...
pub struct GraphicsInterface
{
    ...
    glyth_bind_group_layout: wgpu::BindGroupLayout,
    glyth_render_pipeline: wgpu::RenderPipeline,
...
impl GraphicsInterface
{
    pub fn new(sdl2_system: &SDLSystem) -> Result<Self, String>
    {
        ...
        let glyth_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Glyth Shader"),
            source: wgpu::ShaderSource::Wgsl(include_str!("glyth.wgsl").into()),
        });

        let glyth_bind_group_layout =
        device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                entries: &[
                    wgpu::BindGroupLayoutEntry {
                        binding: 0,
        ...
        let glyth_render_pipeline_layout =
            device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                label: Some("Glyth Render Pipeline Layout"),
                bind_group_layouts: &[&glyth_bind_group_layout],
                push_constant_ranges: &[],
        });

        let glyth_render_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("Glyth Render Pipeline"),
            layout: Some(&glyth_render_pipeline_layout),
            vertex: wgpu::VertexState {
                module: &glyth_shader,
                entry_point: "vs_main",
                buffers: &[wgpu::VertexBufferLayout {
        ...
        Ok(Self{ surface, device, queue, config, 
                 texture_bind_group_layout, sprite_render_pipeline, 
                 glyth_bind_group_layout, glyth_render_pipeline, 
                 rectangle_render_pipeline, 
                 circle_render_pipeline, 
                 clear_color, world_matrix 
                })
    }
    ...
    { 
        match self.internal_batch_render(sprite_textures, sprite_vertices, sprite_indices, 
                                         glyth_textures, glyth_vertices, glyth_indices, 
